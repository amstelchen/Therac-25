...
def main():
    ...
    if curses.LINES < 24 or curses.COLS < 80:
        print(f"Therac-25 needs at least a 80 x 24 terminal. Available: {curses.COLS} x {curses.LINES}. Exiting.")
        sys.exit(1)
...
def display(w):
    ...
    for i in range(9):
        ...
        if i >= 3:
            w.addstr(
                10 + i - 3, RIGHT, "VERIFIED" if actual[i] == prescribed[i] else ""
            )
...
def doBeam():
    ...
    if mode == 0:
        ...
        if actualbeam == beam:
            ...
            w2.addstr(
                1, 2, "MALFUNCTION 54 (%d rads delivered)" % (randint(10000, 20000))
            )  # (rand()%10000+10000))
        else:
            w2.addstr(
                1, 2, "MALFUNCTION 26 (%d rads delivered)" % (randint(10, 20))
            )  # (rand()%10+10))
    w2.refresh()
    #curses.cbreak()
    w2.getch()
